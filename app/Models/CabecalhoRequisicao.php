...
class CabecalhoRequisicao
{
    ...
    public function create(array $data, int $userId, string $category = 'ADMISSAO'): string
{
    ...
    $title = "Requisição " . ($category === 'ADMISSAO' ? 'RAP' : 'RMP') . " - " . ($data['requisitante'] ?? 'Usuário') . " - " . date('Y-m-d H:i');
