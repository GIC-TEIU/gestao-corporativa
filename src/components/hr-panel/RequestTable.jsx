...
// Importa o IncompatibleModal assumindo que ele existe no seu caminho original
...
// Renomeado para RequestTable e adicionado a prop onSelectedItemsChange
...
  const [showIncompatibleModal, setShowIncompatibleModal] = useState(false);
  ...
  // NOVO: Efeito para notificar o componente pai (HRPanel) sobre a lista completa de itens selecionados
  const getSelectedRequisitions = useCallback(() => {
    return data.filter(item => selectedItems.includes(item.id));
  }, [data, selectedItems]);

  useEffect(() => {
    // Garante que o HRPanel receba os objetos completos, não apenas os IDs
...
    // Se for tipo incompatível, mostrar modal e não fazer nada mais
    if (isIncompatibleType && !isSelected) {
      setShowIncompatibleModal(true);
      return;
    }

    // Lógica normal de seleção/deseleção
    if (isSelected) {
...
      } else if (tipoEnvelope.text === selectionType) {
        setSelectedItems([...selectedItems, id]);
      }
    }
  };
...
          {data.map((item) => {
            const isDisabled = selectionType !== null && item.tipoEnvelope.text !== selectionType;
